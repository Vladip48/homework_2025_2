...
QUnit.module('Тестируем функцию factorial', () => {
    ...
    QUnit.test('Факториал для NaN должен выбрасывать ошибку', (assert) => {
        assert.throws(() => {
            factorial(NaN);
        }, /Факториал определен только для целых неотрицательных чисел/, 'Ошибка выбрасывается для NaN');
    });

    QUnit.test('Факториал для Infinity должен выбрасывать ошибку', (assert) => {
        assert.throws(() => {
            factorial(Infinity);
        }, /Факториал определен только для целых неотрицательных чисел/, 'Ошибка выбрасывается для Infinity');
    });
});
